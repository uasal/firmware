...
int8_t BinaryDMDacsCommand(const uint32_t Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	if ( (NULL != Params) && (ParamsLen >= (4 * sizeof(uint32_t))) )
	{
		const uint32_t* DacSetpoints = reinterpret_cast<const uint32_t*>(Params);
                // Setpoint[0] = Board Number
                // Setpoint[1] = Dac Number
                // Setpoint[2] = Dac Channel
                // Setpoint[3] = Dac Value
		printf("\nBinaryDMDacsCommand: 0x%X | 0x%X | 0x%X\n\n", DacSetpoints[0], DacSetpoints[1], DacSetpoints[2], DacSetpoints[3]);
                ...
                //UART_polled_tx_string(&my_uart,(const uint8_t*)DacSetpoints[1]);
                // First need to send the Dac address over spi port
...
int8_t BinaryHVSwitchCommand(const uint32_t Name, char const* Params, const size_t ParamsLen, const void* Argument) {
  if ( (NULL != Params) && (ParamsLen >= (2 * sizeof(uint32_t))) ) {
    const uint32_t* HVParams = reinterpret_cast<const uint32_t*>(Params);
    printf("\nHVSwitch command: %i | %i\n\n", HVParams[0], HVParams[1]);
    // This is where we put in command to ADCs to turn on HV
    SpiADC.HVon_off(HVParams[0],HVParams[1]);  // Param1: board number, params2: switch state (0=off, 1=On)
    
  }
  else {
    printf("\nBinaryHVSwitchCommand: Short packet: %lu (exptected %lu bytes): ", ParamsLen, (2 * sizeof(uint32_t)));
  }
  return(ParamsLen);
}
