...
int MmapHandle = 0;
CGraphFWHardwareInterface* FW = NULL;
...
int main(int argc, char *argv[])
{
    ...
    //~ printf("\n\nFW: Show process resource limits (inf=%llu): ", RLIM_INFINITY);
    ...
    //~ {
        //~ printf("\n\nFW: Initialize thread profiler...");
        //~ wooinit();
    //~ }

    //~ printf("\n\nFW: Load configuration...");

    //~ CreateFWTempFolder();

    //~ printf("\n\nFW: Start Fpga Hardware Manger thread...");
    //~ {
        //~ FpgaProcessor.Init();
    //~ }

	printf("\n\nFW: Connect to hardware...");    
    
	int err = CGraphFWProtoHardwareMmapper::open(MmapHandle, FW);
	
	if (err < 0) { printf("\n\nFW: Coudn't connect to hardware: %d", err); }
