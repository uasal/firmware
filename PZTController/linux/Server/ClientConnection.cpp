...
extern "C"
{
	...
	#endif
	{
		...
		{
			#ifdef WIN32
			formatf("\nZen457Server: connection error: %lu\n", (unsigned long)WSAGetLastError());
			#else
			perror("\nZen457Server: connection error; ");
			#endif
		}
		else { printf("\nZen457Server: Got a connection(%u).\n\n", hsock); }
	}	
	...
	void* SocketHandleClientThread(void *arg)
	{
		...
		printf("\nZen457Server: SocketHandleClientThread: Waiting for a connection...");
		fflush(stdout);
		...
		formatf("\nZen457Server: connected; creating ~another~ socket handler thread for the ~next~ connection...");
		pthread_t thread;
		int rc = pthread_create(&thread, NULL, SocketHandleClientThread, reinterpret_cast<void*>(hServerSocket));
		if (0 != rc) { formatf("\nZen457Server: pthread_create returned %d.\n", rc); }
		...
		while(true)
		{
			...
			{ 
				...
				if (1 != numbytes)
				{
					printf("\nZen457Server: %s: write to socket error (%d bytes gave: %u)\n", Client.Addr.sa_data, numbytes, errno);
					#ifdef WIN32
					closesocket(Client.hSocket);
					#else
					close(Client.hSocket);
					#endif
					printf("\nZen457Server: %s: Closed old socket; looking for new connections.\n\n", Client.Addr.sa_data); 
					SocketConnect(Client.hSocket, Client.hServerSocket, Client.Addr, Client.AddrLen);
				}
				//~ printf("%%");
			...
			{ 
				#ifdef WIN32
				formatf("\nZen457Server: select from socket error: %lu\n", (unsigned long)WSAGetLastError());
				#else
				perror("\nZen457Server: select from socket error: ");
				#endif
				#ifdef WIN32
				closesocket(Client.hSocket);
				#else
			...
			else if (result == 1)
			{
				if (FD_ISSET(Client.hSocket, &sockset)) // The socket has data. For good measure, it's not a bad idea to test further
				{
					...
					if (1 != numbytes)
					{
						printf("\nZen457Server: %s: read from socket error or empty read (%d bytes gave: %u)\n", Client.Addr.sa_data, numbytes, errno);
						perror("\nZen457Server: errno: ");
						#ifdef WIN32
						closesocket(Client.hSocket);
						#else
