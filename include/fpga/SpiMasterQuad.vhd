...
architecture SpiMasterQuad of SpiMasterQuadPorts is
	...
	signal ActiveDataToMosiD : std_logic_vector((BYTE_WIDTH * 8) - 1 downto 0);
...
begin
	...
	ActiveDataToMosiD <= DataToMosiD when DataToMosiLatched = '0' else DataToMosiD_i;
	...
	begin
		...
		if (rst = '1') then
			...
			DataFromMisoD <= std_logic_vector(to_unsigned(0, BYTE_WIDTH * 8));
			XferComplete_i <= '0';
			...
			if ( ( clk'event) and (clk = '1') ) then
				...
				if (ClkDiv < ((CLOCK_DIVIDER / 2) - 1)) then --Since we flop sck back & forth, run divider twice as fast...
					...
					if (SpiBitPos = (BYTE_WIDTH * 8)) then 
						...
						MosiD_i <= ActiveDataToMosiD((BYTE_WIDTH * 8) - 1); 
					...
					if (XferComplete_i = '0') then
						...
						if (Sck_i = ((not(CPOL)) xor CPHA)) then --transition mosi when SCK != CPOL
							...
							if (SpiBitPos > 0) then 
								...
								MosiD_i <= ActiveDataToMosiD(SpiBitPos - 1);
