...
FILE*    mwLog =         NULL;
int      mwFlushing =    0;
int      mwStatLevel =   MW_STAT_DEFAULT;
int      mwNML =         MW_NML_DEFAULT;
int      mwFBI =         0;
long     mwAllocLimit =  0L;
int      mwUseLimit =    0;

long     mwNumCurAlloc = 0L;
mwData*  mwHead = 		NULL;
mwData*  mwTail = 		NULL;
int		mwDataSize =	0;
unsigned char mwOverflowZoneTemplate[] = "mEmwAtch";
int		mwOverflowZoneSize = mwROUNDALLOC;

void     (*mwOutFunction)(int) = NULL;
int      (*mwAriFunction)(const char*) = NULL;
int      mwAriAction = MW_ARI_ABORT;

char     mwPrintBuf[MW_TRACE_BUFFER+8];

unsigned long mwCounter = 0L;
long     mwErrors =      0L;
...
mwStat*  mwStatList = NULL;
long     mwStatTotAlloc = 0L;
long     mwStatMaxAlloc = 0L;
long     mwStatNumAlloc = 0L;
long     mwStatCurAlloc = 0L;
long     mwNmlNumAlloc = 0L;
long     mwNmlCurAlloc = 0L;

mwGrabData* mwGrabList = NULL;
long     mwGrabSize = 0L;

void *   mwLastFree[MW_FREE_LIST];
const char *mwLFfile[MW_FREE_LIST];
int      mwLFline[MW_FREE_LIST];
int      mwLFcur = 0;

mwMarker* mwFirstMark = NULL;
