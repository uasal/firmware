...
#include "cgraph/CGraphDMCIHardwareInterface.hpp"
extern int MmapHandle;
extern CGraphDMCIHardwareInterface* DMCI;	

#include "../MonitorAdc.hpp"
extern CGraphDMCIMonitorAdc MonitorAdc;

#include "../DMCIBuildNum"
...
int8_t VersionCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	if (NULL != DMCI)
	{
		printf("\n\nVersion: Serial Number: %.8X, Global Revision: %s; build number: %u on: %s; fpga build: %u.\n", DMCI->DeviceSerialNumber, GITVERSION, BuildNum, BuildTimeStr, DMCI->FpgaFirmwareBuildNumber);
	}
	else
	{
...
int8_t InitFpgaCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	...
	int err = CGraphDMCIProtoHardwareMmapper::open(MmapHandle, DMCI);
...
int8_t DeInitFpgaCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	...
	int err = CGraphDMCIProtoHardwareMmapper::close(MmapHandle, DMCI);
...
int8_t DMCIDacsCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
    unsigned long A = 0, B = 0, C = 0;	
	if (NULL == DMCI)
	{
		printf("\n\nDMCIDacs: Fpga interface is not initialized! Please call InitFpga first!.");
		return(ParamsLen);
	}
	...
	if (numfound >= 1)
    {
		DMCI->DacASetpoint = A;
		DMCI->DacBSetpoint = A;
		//~ DMCI->DacBSetpoint = 0x006FFFFFUL; //Sometimes this is 100V
		//~ DMCI->DacBSetpoint = 0x00CFFFFFUL; //Aaaaaand, sometimes this is 100V
		DMCI->DacCSetpoint = A;
		printf("\n\nDMCIDacs: set to: %lx, %lx, %lx.\n", A, A, A);
		return(ParamsLen);
    }
	...
	B = DMCI->DacBSetpoint;
	C = DMCI->DacCSetpoint;
	printf("\n\nDMCIDacs: current value: %lx, %lx, %lx.\n", A, B, C);
	
	//Show current A/D values:
	{
		AdcAccumulator A, B, C;
		A = DMCI->AdcAAccumulator;
		B = DMCI->AdcBAccumulator;
		C = DMCI->AdcCAccumulator;
		...
		//~ printf("\n\nDMCIAdcs: current values: 0x%016llx, 0x%016llx, 0x%016llx; %+lld(%u), %+lld(%u), %+lld(%u), %+1.3lf, %+1.3lf, %+1.3lf\n", A.all, B.all, C.all, A.Samples, A.NumAccums, B.Samples, B.NumAccums, C.Samples, C.NumAccums, (4.096 * (A.Samples / A.NumAccums)) / 16777216.0, (4.096 * (B.Samples / B.NumAccums)) / 16777216.0, (4.096 * (C.Samples / C.NumAccums)) / 16777216.0);
		printf("\nDMCIDacs: Sensor A/D's: 0x%016llx, 0x%016llx, 0x%016llx; %+lld(%u), %+lld(%u), %+lld(%u), %+1.3lf, %+1.3lf, %+1.3lf\n", A.all, B.all, C.all, A.Samples, A.NumAccums, B.Samples, B.NumAccums, C.Samples, C.NumAccums, Av, Bv, Cv);
	}
	
	//~ printf("\n\nDMCIdaca: D/A registers at: %u, %u, %u.\n", offsetof(CGraphDMCIHardwareInterface, DacASetpoint), offsetof(CGraphDMCIHardwareInterface, DacBSetpoint), offsetof(CGraphDMCIHardwareInterface, DacCSetpoint));
...
int8_t DMCIAdcsCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	//~ size_t cycle = 0;
	//~ int key = 0;
	...
	if (NULL == DMCI)
	{
		printf("\n\nDMCIAdcs: Fpga interface is not initialized! Please call InitFpga first!.");
		return(ParamsLen);
	}
...
int8_t BISTCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	...
	while(true)
	{
		...
		//~ {
			...
			//~ printf("\n\nBIST: current A/D values: 0x%016llx, 0x%016llx, 0x%016llx; %+lld(%u), %+lld(%u), %+lld(%u), %+1.3lf, %+1.3lf, %+1.3lf; %u, %u, %u.\n", A.all, B.all, C.all, A.Samples, A.NumAccums, B.Samples, B.NumAccums, C.Samples, C.NumAccums, Av, Bv, Cv, offsetof(CGraphDMCIHardwareInterface, AdcAAccumulator), offsetof(CGraphDMCIHardwareInterface, AdcBAccumulator), offsetof(CGraphDMCIHardwareInterface, AdcCAccumulator));
		//~ }
		
		//~ //Update the D/A's every so often
		//~ if (0 == cycle % 4)
		//~ {
			//~ DMCI->DacASetpoint = daca;
			//~ //DMCI->DacBSetpoint = daca;
			//~ DMCI->DacBSetpoint = 0x00CFFFFFUL;
			//~ DMCI->DacCSetpoint = daca;
...
int8_t CirclesCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	...
	while(true)
	{
		...
		{
			DMCI->DacASetpoint = daca;
			DMCI->DacBSetpoint = dacb;
			//~ DMCI->DacBSetpoint = 0x00AAAAAAUL;
			//~ DMCI->DacBSetpoint = 0x006FFFFFUL;
			//~ DMCI->DacBSetpoint = 0x00CFFFFFUL;
			DMCI->DacCSetpoint = dacc;
			//~ printf("\n\nBIST: D/A's set to: %lx, %lx, %lx.\n", daca, 0x006FFFFFUL, dacc);	

			unsigned long rba = DMCI->DacASetpoint;
			unsigned long rbb = DMCI->DacBSetpoint;
			unsigned long rbc = DMCI->DacCSetpoint;
...
int8_t UartCommand(char const* Name, char const* Params, const size_t ParamsLen, const void* Argument)
{
	...
	//~ printf("\nUartCommand: DMCI@0x%p, USR@%u, UF@%u, MAA@%u, ACF@%u, ACF is %u, ", (void*)DMCI, offsetof(CGraphDMCIHardwareInterface, UartStatusRegister), offsetof(CGraphDMCIHardwareInterface, UartFifo), offsetof(CGraphDMCIHardwareInterface, MonitorAdcAccumulator), offsetof(CGraphDMCIHardwareInterface, AdcCFifo), sizeof(AdcFifo));
	printf("\nUartCommand: %u, %u. ", offsetof(CGraphDMCIHardwareInterface, UartFifo2), offsetof(CGraphDMCIHardwareInterface, UartStatusRegister2));
	...
	//~ CGraphDMCIUartStatusRegister UartStatus = DMCI->UartStatusRegister2;
	//~ UartStatus.printf();	
	...
	//~ UartStatus = DMCI->UartStatusRegister; 
	//~ printf("; uart written; ");	
	//~ UartStatus.printf();	
	
	//~ CGraphDMCIUartStatusRegister UartStatus2;
	...
	//~ {
		...
		//~ printf(":%.2X", DMCI->UartFifo);
		//~ UartStatus = DMCI->UartStatusRegister; 		
	//~ }
	
	//~ {
		//~ //clear buffer:
		//~ DMCI->UartStatusRegister.all = 1;
	//~ }
	
	//~ printf(":%.4X", DMCI->UartFifo2);
	...
	if (DMCI) 
	{ 
		Version.SerialNum = DMCI->DeviceSerialNumber; 
		Version.FPGAFirmwareBuildNum = DMCI->FpgaFirmwareBuildNumber; 
	}
