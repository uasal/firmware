...
int MmapHandle = 0;
CGraphDMCIHardwareInterface* DMCI = NULL;
...
//~ MTracer()
//~ {
//~ putenv("MALLOC_TRACE=/home/root/DMCITrace.txt");
...
int main(int argc, char *argv[])
{
    ...
    printf("Welcome to DMCI v%s.b%s.\n", GITVERSION, BUILDNUM);

    printf("\nDMCI: Configure Ram bus wait states...\n");

	//The particulars of connecting the NXP iMX6 processor external memory bus to our FPGA
    eim_setup();

    printf("\n\nDMCI: Configure crash handler...\n");
    {
    ...
    //~ printf("\n\nDMCI: Show process resource limits (inf=%llu): ", RLIM_INFINITY);
    ...
    //~ {
        //~ printf("\n\nDMCI: Initialize thread profiler...");
        //~ wooinit();
    //~ }

    //~ printf("\n\nDMCI: Load configuration...");

    //~ CreateDMCITempFolder();

    //~ printf("\n\nDMCI: Start Fpga Hardware Manger thread...");
    //~ {
        //~ FpgaProcessor.Init();
    //~ }

	printf("\n\nDMCI: Connect to hardware...");    
    
	int err = CGraphDMCIProtoHardwareMmapper::open(MmapHandle, DMCI);
	
	if (err < 0) { printf("\n\nDMCI: Coudn't connect to hardware: %d", err); }
	
	printf("\n\nDMCI: Start User Interface...");    
    ...
    while(true)
    {
		...
		//~ {
			//~ if (NULL != DMCI) 
			//~ {
				//~ CGraphDMCIUartStatusRegister UartStatus = DMCI->UartStatusRegister;
				//~ UartStatus.printf();	
