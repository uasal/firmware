...
struct PinoutMonitorAdc
{
	...
	static uint8_t GetAdcReadChannel()								{ uint8_t val = *(((uint8_t*)DM)+MonitorAdcFpgaAdcChannelAddr); return(val); }
	static void SetAdcReadChannel(const uint8_t val) 					{ *(((uint8_t*)DM)+MonitorAdcFpgaAdcChannelAddr) = (uint8_t)val; }
	//~ old way to hopefully implement later: static void GetAdcSample(Ltc244xAccumulator& val) 				{ val = *((Ltc244xAccumulator*)(((uint8_t*)DM)+MonitorAdcFpgaAdcSampleAddr)); }		
	
	static const size_t spi_timeout = 100;
	
  //	static bool busy() { return(0 == (DM->MonitorAdcSpiCommandStatusRegister.TransactionComplete) ); }
  ...
  //	static void enable(const bool en) { DM->MonitorAdcSpiCommandStatusRegister.all = (uint32_t)en; }
	
	static void transmit(const uint8_t val) 					
	{ 
		waitbusytimeout();
                //		DM->MonitorAdcSpiTransactionRegister = (uint32_t)val; 
	}
	...
	{ 
		...
		//readback
		waitbusytimeout();
                //	out = DM->MonitorAdcSpiTransactionRegister; 
	...
	static bool nDrdy() 				
	{ 
          //	return(0 == (DM->MonitorAdcSpiCommandStatusRegister.nDrdy) );
	}		
...
public:
	...
	CGraphDMMonitorAdc() : AdcFound(false), Monitor(false),
  ...
  //	ads1258<PinoutMonitorAdc> Adc;
	...
	//~ static uint8_t GetAdcReadChannel()								{ uint8_t val = *(((uint8_t*)DM)+MonitorAdcFpgaAdcChannelAddr); return(val); }
	//~ static void SetAdcReadChannel(const uint8_t val) 					{ *(((uint8_t*)DM)+MonitorAdcFpgaAdcChannelAddr) = (uint8_t)val; }
	//~ static void GetAdcSample(Ltc244xAccumulator& val) 				{ val = *((Ltc244xAccumulator*)(((uint8_t*)DM)+MonitorAdcFpgaAdcSampleAddr)); }		
